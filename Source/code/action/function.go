...
func (s *FunctionSetting) IsDefault() bool {
	return s.VMixAddress == "" && s.Function == "" && s.ActsEvent == "" && s.ActInput == "" && s.ActsActiveState == "" && s.ActsInactiveState == ""
}
...
type functionAction struct {
	...
	store             setting.SettingStore[*FunctionSetting]
...
func (f *functionAction) OnWillAppear() streamdeck.EventHandler {
	return func(ctx context.Context, client *streamdeck.Client, event streamdeck.Event) error {
		...
		payload := streamdeck.WillAppearPayload[*FunctionSetting]{}
		...
		f.connectionManager.AddContext(ctx, payload.Settings.VMixAddress, event.Context, FunctionActionUUID)
		...
		if err := f.client.SetImage(ctx, "", streamdeck.HardwareAndSoftware); err != nil {
			f.logger.Error(ctx, "Failed to set image %v", err)
		}
...
func (f *functionAction) OnWillDisappear() streamdeck.EventHandler {
	return func(ctx context.Context, client *streamdeck.Client, event streamdeck.Event) error {
		...
		payload := streamdeck.WillDisappearPayload[FunctionSetting]{}
		if err := json.Unmarshal(event.Payload, &payload); err != nil {
			f.logger.Error(ctx, "Failed to unmarshal payload", "error", err)
			return nil
		}
...
func (f *functionAction) OnUpdateSettings() streamdeck.EventHandler {
	return func(ctx context.Context, client *streamdeck.Client, event streamdeck.Event) error {
		payload := streamdeck.DidReceiveSettingsPayload[FunctionSetting]{}
		if err := json.Unmarshal(event.Payload, &payload); err != nil {
			f.logger.Error(ctx, "Failed to unmarshal payload", "error", err)
			return nil
		}
		...
		if err := f.client.SetImage(ctx, "", streamdeck.HardwareAndSoftware); err != nil {
			f.logger.Error(ctx, "Failed to set image", "error", err)
		}
...
func (f *functionAction) OnKeyDown() streamdeck.EventHandler {
	return func(ctx context.Context, client *streamdeck.Client, event streamdeck.Event) error {
		...
		payload := streamdeck.KeyDownPayload[*FunctionSetting]{}
		if err := json.Unmarshal(event.Payload, &payload); err != nil {
			f.logger.Error(ctx, "Failed to unmarshal payload", "error", err)
			return nil
		}
